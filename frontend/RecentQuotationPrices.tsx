...
export default function RecentQuotationPrices({ line, customer, quotationId, t, onClose }: { line: any; customer: any; quotationId?: string; t: Translate; onClose: () => void }) {
  const [stage, setStage] = useState<"DRAFT" | "ISSUED">("ISSUED");
  ...
  useEffect(() => {
    ...
    const query = new URLSearchParams({ itemKey: priceHistoryItemKey(line), stage, customerNumber: customer?.number ?? "", customerName: customer?.name ?? "", page: String(page), pageSize: "10" });
  ...
  return <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><section className="card modal recent-price-modal" role="dialog" aria-modal="true">
    ...
    <div className="tabs"><button className={stage === "ISSUED" ? "primary" : ""} onClick={() => { setStage("ISSUED"); setPage(0); }}>{t("Issued quotations", "العروض المصدرة")}</button><button className={stage === "DRAFT" ? "primary" : ""} onClick={() => { setStage("DRAFT"); setPage(0); }}>{t("Draft prices", "أسعار المسودات")}</button></div>
    {loading ? <p>{t("Loading…", "جارٍ التحميل…")}</p> : error ? <div className="notice error">{error}</div> : !data.items.length ? <div className="notice">{t("No previous prices for this item.", "لا توجد أسعار سابقة لهذا الصنف.")}</div> : <div className="table-scroll"><table><thead><tr><th>{t("Quotation", "العرض")}</th><th>{t("Customer", "العميل")}</th><th>{t("Staff / date", "الموظف / التاريخ")}</th><th>{t("Qty / level", "الكمية / المستوى")}</th><th>{t("Rate", "النسبة")}</th><th>{t("Unit excl./incl. VAT", "الوحدة قبل/بعد الضريبة")}</th></tr></thead><tbody>{data.items.map((x: any) => <tr key={x.id} className={x.same_customer ? "same-customer-price" : ""}><td><b>{x.quotation_number}</b>{x.same_customer && <span className="badge success">{t("Same customer", "نفس العميل")}</span>}</td><td>{x.customer_name || "—"}</td><td>{x.staff_name}<br/><small>{new Date(x.last_seen_at).toLocaleString()}</small></td><td>{x.quantity}<br/><small>{x.selling_level}</small></td><td>{x.adjustment_mode === "MARKUP" ? `${Number(x.effective_markup)}% ${t("markup", "زيادة")}` : `${Number(x.effective_discount)}% ${t("discount", "خصم")}`}</td><td><b>SAR {x.final_excl}</b><br/><small>SAR {x.final_incl}</small></td></tr>)}</tbody></table></div>}
