...
export default function QuotationTemplates({ t, cart, onUse }: { t: Translate; cart: any; onUse: (cart: any, message?: string) => void }) {
  ...
  return <section className="stack">
    ...
    {selected && <div className="modal-backdrop"><section className="card modal">
      ...
      <fieldset><legend>{t("Include fields", "الحقول المضمنة")}</legend>{["name", "number", "mobile", "reference", "notes"].map((k) => <label key={k}><input type="checkbox" checked={selected.includeFields.includes(k)} onChange={(e) => setSelected({ ...selected, includeFields: e.target.checked ? [...selected.includeFields, k] : selected.includeFields.filter((x: string) => x !== k) })}/>{k}</label>)}</fieldset>
