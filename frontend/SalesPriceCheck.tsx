...
export default function SalesPriceCheck({ t }: { t: Translate }) {
  const [tab, setTab] = useState<"CHECKS" | "HISTORICAL">("CHECKS");
  const [reports, setReports] = useState<any[]>([]);
  const [historicalLists, setHistoricalLists] = useState<any[]>([]);
  const [report, setReport] = useState<any>(null);
  const [mapping, setMapping] = useState({ partNumber: "", salesPrice: "", description: "", saleDate: "", historicalListId: "" });
  ...
  const load = () => {
    ...
    return api("sales-price-checks").then(setReports);
  };
  const open = async (
    id: string,
    p = 0,
    ...
    setMapping(
      Object.keys(nextReport.mapping || {}).length
        ...
        : {
            ...
            description: autoMap(nextReport.columns || [], [
              "description",
              "itemname",
              "title",
            ]),
            saleDate: autoMap(nextReport.columns || [], [
              "saledate",
              "invoicedate",
              "date",
            ]),
  ...
  const discountDisplay = (value: unknown) => {
    if (value === null || value === undefined || value === "") return "—";
    const num = Number(value);
    if (num < 0) return `+${Math.abs(num).toFixed(2)}% ${t("Markup", "زيادة")}`;
  ...
  return (
    <div className="sales-check">
      ...
      <div className="actions wrap" style={{ marginBottom: "1rem" }}>
        {[
          ["CHECKS", "Checks", "الفحوصات"],
          ["HISTORICAL", "Historical Lists", "قوائم التاريخ"],
        ].map(([key, en, ar]) => (
          <button
            key={key}
            ...
            <div className="sales-check-setup">
              <div className="mapping-grid">
                {[
                  ...
                  ["saleDate", "Sale date (Optional)", "تاريخ البيع (اختياري)"],
                ].map(([key, en, ar]) => (
                  <label key={key}>
                    {t(en, ar)}
                    <select
              ...
              <div className="mapping-grid" style={{ marginTop: "1rem", borderTop: "1px solid var(--border)", paddingTop: "1rem" }}>
                <label>
                  {t("Compare with Historical Prices (Optional)", "مقارنة مع الأسعار التاريخية (اختياري)")}
                  <select
                    disabled={busy || reportRunning}
                    value={(mapping as any).historicalListId || ""}
                    onChange={(e) => setMapping({ ...mapping, historicalListId: e.target.value })}
                  >
                    <option value="">{t("Use current app prices", "استخدام أسعار التطبيق الحالية")}</option>
                    {historicalLists.map((l) => (
                    ...
                    {report.rows.map((row: any) => (
                      <tr key={row.id}>
                        ...
                        <td data-label="Discount %" className="number-cell">
                          {discountDisplay(row.discount_percent)}
                        </td>
...
function HistoricalPrices({ t, run, busy }: { t: Translate, run: any, busy: boolean }) {
  const [lists, setLists] = useState<any[]>([]);
  const [mappingId, setMappingId] = useState("");
  const [mappingCols, setMappingCols] = useState<string[]>([]);
  const [mappingName, setMappingName] = useState("");
  const [mapping, setMapping] = useState({ partNumber: "", validFrom: "", validTo: "", price: "" });

  const load = () => api("historical-prices").then(setLists);
  ...
  return (
    <div className="historical-prices-section">
      {!mappingId ? (
        <>
          <form
            className="actions wrap sales-check-toolbar sales-check-upload-bar"
            onSubmit={(e) => {
              ...
              const form = new FormData(e.currentTarget);
              run(async () => {
                const res = await api("historical-prices", "POST", form);
                setMappingId(res.id);
                setMappingCols(res.columns);
                setMappingName(res.name);
                setMapping({
                  partNumber: autoMap(res.columns, ["itemcode", "partnumber", "part"]),
                  validFrom: autoMap(res.columns, ["validfrom", "fromdate", "start"]),
                  validTo: autoMap(res.columns, ["validto", "todate", "end"]),
                  price: autoMap(res.columns, ["price", "salesprice", "amount"]),
                });
              });
            }}
          >
            <input name="file" type="file" accept=".xls,.xlsx,.csv" required />
            <button className="primary" disabled={busy}>
              {t("Upload historical prices", "رفع أسعار تاريخية")}
            </button>
          </form>
                ...
                {lists.length === 0 && (
                  <tr>
                    <td colSpan={4} style={{ textAlign: "center", padding: "2rem" }}>
                      {t("No historical lists uploaded yet.", "لم يتم رفع قوائم تاريخية بعد.")}
                    </td>
                  </tr>
                )}
                {lists.map((l) => (
        ...
        <section className="card inset-card sales-check-shell">
          <div className="sales-check-report-head">
            <div>
              <h3>{mappingName} - {t("Column Mapping", "ربط الأعمدة")}</h3>
            </div>
            <div className="actions wrap">
              <button disabled={busy} onClick={() => setMappingId("")}>
                {t("Cancel", "إلغاء")}
              </button>
            </div>
          </div>
          <div className="sales-check-setup">
            <div className="mapping-grid">
              {[
                ["partNumber", "Part number", "رقم الصنف"],
                ["price", "Price", "السعر"],
                ["validFrom", "Valid from (Optional)", "صالح من (اختياري)"],
                ["validTo", "Valid to (Optional)", "صالح إلى (اختياري)"],
              ].map(([key, en, ar]) => (
                <label key={key}>
                  {t(en, ar)}
                  <select
            ...
            <div className="actions wrap">
              <button
                className="primary"
                disabled={busy || !mapping.partNumber || !mapping.price}
                onClick={() =>
                  run(async () => {
                    await api(`historical-prices/${mappingId}?action=map`, "POST", mapping);
                    setMappingId("");
                    await load();
                  })
                }
              >
                {t("Save mapping", "حفظ الربط")}
              </button>
            </div>
          </div>
        </section>
