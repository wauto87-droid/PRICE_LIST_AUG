...
export async function exportReport(db: DB, actor: Actor, raw: any) {
  const format = raw.format === "PDF" ? "PDF" : "XLSX",
    customers = raw.scope === "CUSTOMERS",
    lines = !customers && raw.filters?.view === "LINES",
    ar = raw.language === "ar";
  const report = await list(
    ...
    {
      ...raw.filters,
      view: customers ? "CUSTOMERS" : lines ? "LINES" : "NOTES",
    },
    true,
  );
  const label = (en: string, arabic: string) => (ar ? arabic : en);
  const billingLabel = (key: string) =>
    (billingNames[key] || ["Unknown billing status", "حالة فوترة غير معروفة"])[
      ar ? 1 : 0
    ];
  const stageLabel = (key: string) =>
    (stageNames[key] || ["Unknown stage", "مرحلة غير معروفة"])[ar ? 1 : 0];
  const headings = customers
    ? [
        label("Customer", "العميل"),
        label("DNs", "الأذونات"),
        label("Unbilled DNs", "أذونات غير مفوترة"),
        label("Outstanding rows", "صفوف متبقية"),
        label("Oldest DN", "أقدم إذن"),
      ]
    : lines
      ? [
          label("Source row", "صف المصدر"),
          label("Date", "التاريخ"),
          label("DN", "الإذن"),
          label("Customer", "العميل"),
          label("Item", "الصنف"),
          ...
          label("Invoice return", "مرتجع الفاتورة"),
          label("Delivery return", "مرتجع التسليم"),
          label("Balance", "المتبقي"),
          label("Billing", "الفوترة"),
          label("Follow-up", "المتابعة"),
        ]
      : [
          label("Customer", "العميل"),
          label("DN", "الإذن"),
  ...
  const rows: any[][] = customers
    ? report.rows.map((r) => [
        ...
        r.unbilled,
        r.outstanding,
        displayDate(r.oldest),
      ])
    : lines
      ? report.rows.map((r) => [
          r.row,
          displayDate(r.doc_date),
          r.doc_no,
          r.customer,
          r.itemCode,
          r.itemName,
  ...
  if (format === "XLSX") {
    ...
    const sheet = book.addWorksheet(
      customers ? "Customers" : "Delivery notes",
      { views: [{ state: "frozen", ySplit: 1, rightToLeft: ar }] },
    );
    ...
    sheet.getRow(1).fill = {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb: "FF174B57" },
    };
    sheet.columns.forEach((col, i) => {
      col.width = i === 0 || i === 4 ? 34 : 18;
    });
    const dateCol = customers ? 5 : lines ? 2 : 3;
    sheet.eachRow((row, i) => {
      if (i > 1) {
        const cell = row.getCell(dateCol);
        const val = String(cell.value);
        cell.value = new Date(
          val.split("-").reverse().join("-") + "T00:00:00Z",
        );
        cell.numFmt = "dd-mm-yyyy";
      }
    });
    const context = book.addWorksheet("Report context");
    context.addRows([
      ["Report", report.report.name],
      ["Filters", JSON.stringify(report.filters)],
      [
        "Billing",
        "Imported evidence; follow-up moves do not change billing figures",
      ],
    ]);
    return {
