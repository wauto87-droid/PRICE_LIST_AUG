...
export async function salesCheckHtml(db: DB, reportId: string) {
  ...
  const body = rows
    .map(
      (r: any) =>
        `<tr class="${r.status === "UNMATCHED" ? "bad" : r.status !== "MATCHED" ? "warn" : ""}"><td>${r.row_number}</td><td>${escapeHtml(r.source_part || "")}</td><td>${escapeHtml(r.matched_part || "")}</td><td>${escapeHtml(r.description || "")}</td><td class="num">${display(r.sales_price)}</td><td class="num">${display(r.list_price)}</td><td class="num">${display(r.discount_percent)}</td><td>${r.product_id ? "Checked" : "Not matched item"}</td><td>${escapeHtml(r.status)}${r.error ? ` - ${escapeHtml(r.error)}` : ""}</td></tr>`,
    )
    .join("");
