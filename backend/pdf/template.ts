...
export function quotationHtml(q: any, settings: any, logo: string) {
 ...
 <table><thead><tr><th>Part / الصنف</th><th>Description / الوصف</th><th>Qty / الكمية</th><th>Unit ${incl ? "incl." : "excl."} VAT${both ? "<small>Incl. VAT</small>" : ""}</th><th>Discount<br>الخصم</th><th>Excl. VAT<br>قبل الضريبة</th><th>VAT<br>الضريبة</th><th>Total<br>الإجمالي</th></tr></thead><tbody>${q.lines.map((l: any) => {
  ...
  return `<tr><td>${e(l.partNumber)}${l.source === "CUSTOM" || l.input?.type === "CUSTOM" ? "<small>Custom / مخصص</small>" : ""}${deliveryInfo}</td><td>${e(l.description)}</td><td class="num">${e(l.price.quantity)} ${e(l.unit)}</td><td class="num">${e(incl ? l.price.finalIncl : l.price.finalExcl)}${both ? `<small>${e(l.price.finalIncl)}</small>` : ""}</td><td class="num">${e(l.price.effectiveDiscount)}%</td><td class="num">${e(l.price.subtotal)}</td><td class="num">${e(l.price.vatAmount)}<small>${e(l.price.vatRate)}%</small></td><td class="num">${e(l.price.total)}</td></tr>`;
}).join("")}</tbody></table>
 <div class="totals"><div><span>Subtotal / المجموع</span><span>${e(q.totals.subtotal)}</span></div><div><span>VAT / الضريبة</span><span>${e(q.totals.vat)}</span></div><div class="grand"><span>Total / الإجمالي</span><span>${e(s.currency)} ${e(q.totals.total)}</span></div></div>${customerNotes}<div class="terms">${terms}</div><footer>${e(config.footer)}<br>Quotation only — not a tax invoice. / عرض سعر فقط — ليس فاتورة ضريبية.<br>Unit VAT-inclusive values are rounded for display. Totals use line-level VAT rounding.</footer></main></body></html>`;
}
