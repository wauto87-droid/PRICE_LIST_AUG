...
export function quotationHtml(q: any, settings: any, logo: string) {
  ...
  const terms = s.quotation
    ...
    : "";
  const customerNotes = q.customer.notes
    ? `<div class="customer-notes"><strong>Notes / ملاحظات</strong><span dir="auto">${e(q.customer.notes)}</span></div>`
    : "";
 ...
 @page{size:A4;margin:11mm 12mm}*{box-sizing:border-box}body{font-family:'Noto Sans Arabic',Arial,sans-serif;color:#202027;font-size:10.5px;font-weight:600;margin:0}main{position:relative;z-index:1}header{display:flex;align-items:center;justify-content:space-between;gap:12px;border-bottom:3px solid #cc0000;padding-bottom:10px}header img{width:70px;height:78px;object-fit:contain}h1{font-size:20px;margin:0}h2{font-size:15px;color:#cc0000;margin:4px 0}.arabic{direction:rtl}section{margin:12px 0;display:flex;justify-content:space-between;gap:15px}table{width:100%;border-collapse:collapse}th{background:#f4f4f6;text-align:start;font-size:9.5px}td,th{padding:6px 5px;border-bottom:1px solid #ddd}thead{display:table-header-group}tr{break-inside:avoid}td{overflow-wrap:anywhere}.num{text-align:right;white-space:nowrap;font-variant-numeric:tabular-nums}.totals{margin:12px 0 0 auto;width:280px;break-inside:avoid}.totals div{display:flex;justify-content:space-between;padding:6px}.grand{background:#fff0f0;font-size:15px;border-top:2px solid #cc0000;font-weight:800}.customer-notes{display:block;margin:8px 0 0;padding:7px 10px;border:1px solid #ddd;border-left:3px solid #cc0000;white-space:pre-wrap;overflow-wrap:anywhere;break-inside:avoid}.customer-notes strong{display:block;margin-bottom:3px}footer{margin-top:6px;color:#555;border-top:1px solid #ddd;padding-top:5px;font-size:7.5px;line-height:1.2}.footer-separator{padding:0 3px}.draft{color:#cc0000}small{display:block;color:#555;font-size:8.5px;font-weight:700}.watermark{position:fixed;left:50%;z-index:0;pointer-events:none;text-align:center;font-size:32px;font-weight:800;overflow-wrap:anywhere}.watermark img{width:100%;height:auto;max-height:450px;object-fit:contain}.company,.terms{white-space:pre-wrap;overflow-wrap:anywhere;line-height:1.55}.company{margin-top:8px}.terms{margin-top:12px}.terms>div{margin:3px 0}.validity{font-size:9px}.no-print{padding:12px;background:#fff3d6}@media print{.no-print{display:none}}
 </style></head><body>${watermark}<main><header><img src="${e(logo)}" alt="AMT Electric"><div><h1>${e(s.companyName)}</h1><div class="arabic">${e(s.companyArabic)}</div><h2>QUOTATION / عرض سعر</h2></div><div><strong>${e(q.number)}</strong><br>${e(new Intl.DateTimeFormat("en-GB", { timeZone: "Asia/Riyadh", dateStyle: "medium" }).format(new Date(q.issued_at ?? q.created_at)))}<br><small class="validity">Valid for / الصلاحية: ${e(`${config.validityDays} days / أيام`)}</small><span class="draft">${q.status === "DRAFT" ? "DRAFT · مسودة" : ""}</span></div></header>
 <div class="company">${company}</div><section><div><strong>Customer / العميل</strong><br>${e(q.customer.name || "Walk-in Customer")}<br>${e(q.customer.number)} ${e(q.customer.mobile)}</div><div>Reference / المرجع<br>${e(q.customer.reference)}</div></section>
