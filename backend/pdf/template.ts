...
export function quotationHtml(q: any, settings: any, logo: string) {
 ...
 <div class="totals"><div><span>Total Quantity / إجمالي الكمية</span><span>${e(totalQuantity)}</span></div><div><span>Subtotal / المجموع</span><span>${e(q.totals.subtotal)}</span></div><div><span>VAT / الضريبة</span><span>${e(q.totals.vat)}</span></div><div class="grand"><span>Total / الإجمالي</span><span>${e(s.currency)} ${e(q.totals.total)}</span></div></div>${customerNotes}<div class="terms">${terms}</div><footer>${e(config.footer)}<br>Quotation only — not a tax invoice. / عرض سعر فقط — ليس فاتورة ضريبية.<br>Unit VAT-inclusive values are rounded for display. Totals use line-level VAT rounding.</footer></main></body></html>`;
}
