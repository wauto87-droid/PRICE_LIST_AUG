...
export function quotationHtml(q: any, settings: any, logo: string) {
 ...
 <table><thead><tr><th>Part / الصنف</th><th>Description / الوصف</th><th>Qty / الكمية</th><th>Unit ${incl ? "incl." : "excl."} VAT${both ? "<small>Incl. VAT</small>" : ""}</th><th>Discount<br>الخصم</th><th>Excl. VAT<br>قبل الضريبة</th><th>VAT<br>الضريبة</th><th>Total<br>الإجمالي</th></tr></thead><tbody>${q.lines.map((l: any) => `<tr><td>${e(l.partNumber)}${l.source === "CUSTOM" || l.input?.type === "CUSTOM" ? "<small>Custom / مخصص</small>" : ""}</td><td>${e(l.description)}</td><td class="num">${e(l.price.quantity)} ${e(l.unit)}</td><td class="num">${e(incl ? l.price.finalIncl : l.price.finalExcl)}${both ? `<small>${e(l.price.finalIncl)}</small>` : ""}</td><td class="num">${e(l.price.effectiveDiscount)}%</td><td class="num">${e(l.price.subtotal)}</td><td class="num">${e(l.price.vatAmount)}<small>${e(l.price.vatRate)}%</small></td><td class="num">${e(l.price.total)}</td></tr>`).join("")}</tbody></table>
