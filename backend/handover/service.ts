...
export async function correctDeliveryImport(db: DB, actor: Actor, id: string, raw: unknown) {
  ...
  try {
    await db.transaction(async tx => {
      await tx.query("INSERT INTO delivery_quote_jobs(id,filename,file_path,status,owner_id,mapping,summary,source_job_id) VALUES($1,$2,$3,'AWAITING_MAPPING',$4,$5,$6,$7)", [nextId, source.filename, target, actor.id, json(source.mapping), json(source.summary), id]);
...
const historySchema = z.object({ itemKey: z.string().min(1).max(250), stage: z.enum(["DRAFT", "ISSUED"]), customerNumber: z.string().trim().max(80).default(""), customerName: z.string().trim().max(150).default(""), excludeQuotationId: z.string().uuid().optional(), page: z.coerce.number().int().min(0).default(0), pageSize: z.coerce.number().int().min(1).max(50).default(10) });
